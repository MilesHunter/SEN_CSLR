...
class BaseFeeder(data.Dataset):
    def __init__(self, prefix, gloss_dict, dataset='phoenix2014', drop_ratio=1, num_gloss=-1, mode="train", transform_mode=True,
                 datatype="lmdb", frame_interval=1, image_scale=1.0, kernel_size=1, allowable_vid_length=16, input_size=224):
        self.mode = mode
        self.ng = num_gloss
        self.prefix = prefix
    ...
    def read_video(self, index):
        ...
        if len_img <=self.allowable_vid_length * total_stride:
            vid_path = fi['original_info']
            print(f'Video length shorter than expected {len_img}/{self.allowable_vid_length * total_stride} : {vid_path}')  #raise ValueError
        '''
